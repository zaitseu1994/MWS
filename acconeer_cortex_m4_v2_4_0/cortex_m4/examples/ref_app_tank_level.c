...
bool acc_ref_app_tank_level(void)
{
	...
	{
		fprintf(stderr, "Failed to create detector configuration\n");
		acc_rss_deactivate();
		return false;
	}
	...
	{
		fprintf(stderr, "Failed to calibrate detector\n");
		acc_detector_distance_configuration_destroy(&distance_configuration);
	...
	{
		fprintf(stderr, "Failed to calibrate sensor\n");
		return false;
	}
	...
	while (true)
	{
		...
		bool status = measure_close_range(&distance_handle, distance_configuration, &distance_detected, &distance);
                           ...
                           acc_detector_distance_configuration_t distance_configuration)
{
	...
	acc_detector_distance_configuration_maximize_signal_attenuation_set(distance_configuration, true);
                       ...
                       float *range_gain, uint16_t *background, uint16_t background_length)
{
	...
	{
		fprintf(stderr, "Unable to record background without data saturation\n");
		status = false;
	}
                 ...
                 acc_detector_distance_result_info_t   *result_info)
{
	...
	while (gain > 0)
	{
		...
		{
			fprintf(stderr, "Failed to get next from detector\n");
			return false;
		}
	...
	{
		fprintf(stderr, "Unable to measure without data saturation\n");
		return false;
	}
